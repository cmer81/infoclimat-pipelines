...
pub fn write_spatial_omfile<G: Grid>(
    ...
    let arr_offset = writer
        .write_array(finalized, ANOMALY_VARIABLE, &[meta_offset])
...
pub fn read_spatial_omfile(path: &Path) -> Result<(Array2<f32>, OmfileMetadata)> {
    let path_str = path
        .to_str()
    ...
    let var = root
        .get_child_by_name(ANOMALY_VARIABLE)
        .with_context(|| format!("variable {ANOMALY_VARIABLE} absente dans {path:?}"))?;
    let arr = var
        .expect_array()
